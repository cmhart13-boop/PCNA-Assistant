...
.phone{min-height:100vh;background:#fff}.top{height:94px;display:grid;grid-template-columns:52px 1fr 52px;align-items:center;padding:14px 8px 0}
.menu,.bell{font-size:30px;color:var(--navy);text-align:center}.logo{font-size:42px;line-height:1;font-weight:1000;letter-spacing:-5px;text-align:center;color:#05377d}.logo small{font-size:9px;letter-spacing:0;margin-left:5px}
.hero{height:195px;border-radius:16px;overflow:hidden;background:#062e63;position:relative;box-shadow:0 4px 14px #001e4933;margin-bottom:16px}.hero img{width:100%;height:100%;object-fit:cover}.hero:after{content:"";position:absolute;inset:0;background:linear-gradient(90deg,#031f4ae6 0%,#062d5988 46%,transparent 72%)}
...
.mobile-note{display:none}@media(min-width:700px){.phone{box-shadow:0 0 50px #122b4b26}.mobile-note{display:block;text-align:center;font-size:12px;color:#627087;padding:8px}.block-container{max-width:548px!important}}
...
if st.session_state.view == "home":
    st.markdown(f'''<div class="phone"><div class="top"><div class="menu">☰</div><div class="logo">PCNA<small>TM</small></div><div class="bell">♧</div></div>
    <div class="hero"><img src="{HERO}" alt="PCNA products"><div class="hero-copy"><h1>Branded.<br>Merchandise.<br>Delivered.</h1><p>Explore thousands of promotional products to elevate your brand.</p><span class="shop">SHOP NOW</span></div></div>
    <h2 class="section-title">What do you need?</h2><div class="cards">''', unsafe_allow_html=True)
    cards = [
    ...
    st.markdown('<div class="top"><div class="menu">☰</div><div class="logo">PCNA<small>TM</small></div><div class="bell">♧</div></div>', unsafe_allow_html=True)
